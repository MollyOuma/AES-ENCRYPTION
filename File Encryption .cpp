...
WCHAR szTitlemain[MAX_LOADSTRING];                  // The title bar text
WCHAR szWindowClassmain[MAX_LOADSTRING];            // the main window class name

HBRUSH bkbrush = NULL;
COLORREF bkcolor = RGB(255, 255, 255);

LPCWSTR szTitleprogress = L"progress";
LPCWSTR szWindowClassprogress = L"title";


// Forward declarations of functions included in this code module:
ATOM                MyRegisterClass(HINSTANCE hInstance, WNDPROC  WndProcedure, LPCWSTR windowname, DWORD menu);
HWND                InitInstance(HINSTANCE, int, RECT window_rectangle, LPCWSTR windowname, LPCWSTR windowtitle, HWND hwnd, DWORD ch);
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDS_APP_TITLE, szTitlemain, MAX_LOADSTRING);
    LoadStringW(hInstance, IDC_FILEENCRYPTION, szWindowClassmain, MAX_LOADSTRING);

    MyRegisterClass(hInstance, WndProc, szWindowClassmain, IDC_FILEENCRYPTION);


    // Perform application initialization:
    hWnd = InitInstance(hInstance, nCmdShow, window_rectangle,szWindowClassmain, szTitlemain, nullptr,WS_OVERLAPPEDWINDOW);
    if (!hWnd)
          return FALSE;
    else
    {
...
//
HWND InitInstance(HINSTANCE hInstance, int nCmdShow, RECT window_rectangle, LPCWSTR windowname, LPCWSTR windowtitle, HWND parent, DWORD ch)
{
   hInst = hInstance; // Store instance handle in our global variable

  HWND hwnd = CreateWindowW(windowname, windowtitle, ch, window_rectangle.left, window_rectangle.top,
        ...
        {  
                ...
                //encryption
            {
                ...
                if (filepath_valid == true && password_length == 16)
                {
                    ...
                    start_job();
                    {
                        MessageBox(hWnd, L"Done!", L"Cipher", MB_ICONINFORMATION | MB_SYSTEMMODAL);
                        done = false;
                    }
                }
                else
                    MessageBox(hWnd, L"You entered invalid details", L"Cipher", MB_ICONERROR);
                break;
            }
...
INT_PTR CALLBACK Info(HWND hWndDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        int left = (window_rectangle.right - window_rectangle.left) / 2 - (info_dialog_rectangle.right / 2);
        int top = (window_rectangle.bottom - window_rectangle.top) / 2 + info_dialog_rectangle.bottom / 2;

        GetWindowRect(hWndDlg, &info_dialog_rectangle);
        SetWindowPos(hWndDlg, NULL, left, top, info_dialog_rectangle.right, 170, SWP_SHOWWINDOW);
...
{    
    ...
    SendMessage(hwndprogress, PBM_SETRANGE, 0, MAKELPARAM(0, end));
